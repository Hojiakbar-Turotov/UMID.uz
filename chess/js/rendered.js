"use strict";
            ...
            static getPositionsFromShortCode(shortCode) {
                ...
                for (let id in positions) {
                    if (overrides[id]) {
                        ...
                        positions[id] = defaultPositionMode ? positions[id] : {
                            active: false
                        };
                    }
                }
      ...
      <use href="#${piece}" />
        ...
        class Constraints {
            static generate(args, resultingChecks) {
                ...
                const result = method ? method(args) : {
                    moves: [],
                    captures: []
                };
                if (resultingChecks) {
                    const moveIndex = args.moveIndex + 1;
                    result.moves = result.moves.filter((location) => !resultingChecks({
                        piece,
                        location,
                        capture: false,
                        moveIndex
                    }).length);
                    result.captures = result.captures.filter((location) => !resultingChecks({
                        piece,
                        location,
                        capture: true,
                        moveIndex
        ...
        class Piece {
            ...
            dir(stepsRow, stepsColumn, positions) {
                ...
                if (row >= 0 && row <= 7 && col >= 0 && col <= 7) {
                    return {
                        row: Utils.intToRow(row),
                        col: Utils.intToCol(col)
                    };
                }
                return undefined;
            }
        ...
        class Board {
            ...
            pieceCalculateMoves(pieceId, moveIndex, state, piecePositions, piecesTilesCaptures, piecesTilesMoves, tilesPiecesCaptures, tilesPiecesMoves, resultingChecks, kingCastles) {
                const {
                    captures,
                    moves
                } = this.pieces[pieceId].options(moveIndex, state, this.pieces, piecePositions, resultingChecks, kingCastles);
                ...
                if (this.active) {
                    if (this.activePieceOptions.length) {
                        const {
                            col,
                            row
                        } = this.activePieceOptions[Math.floor(Math.random() * this.activePieceOptions.length)];
                        return {
                            col,
                            row
                        };
                        ...
                        const {
                            col,
                            row
                        } = this.board.piecePositions[this.active.data.id];
                        return {
                            col,
                            row
                        };
        ...
        class View {
            ...
            drawActivePiece(activePieceId) {
                const {
                    row,
                    col
                } = this.game.board.piecePositions[activePieceId];
            ...
            drawPiecePositions(moves = [], moveInner = "") {
                ...
                this.game.board.tileEach(({
                    row,
                    col
                }, piece, pieceMoves, pieceCaptures) => {
      ...
      `;
            ...
            drawPositions(moves, captures) {
                moves === null || moves === void 0 ? void 0 : moves.forEach(({
                    row,
                    col
                }) => {
                    var _a, _b;
                    this.tiles[row][col].classList.add("highlight-move");
                    (_b = this.pieces[(_a = this.game.board.tileFind({
                        row,
                        col
                    })) === null || _a === void 0 ? void 0 : _a.data.id]) === null || _b === void 0 ? void 0 : _b.classList.add("highlight-move");
                });
                captures === null || captures === void 0 ? void 0 : captures.forEach(({
                    row,
                    col,
                    capture
                }) => {
                    ...
                    (_b = this.pieces[(_a = this.game.board.tileFind({
                        row,
                        col
                    })) === null || _a === void 0 ? void 0 : _a.data.id]) === null || _b === void 0 ? void 0 : _b.classList.add("highlight-capture");
                });
            }
            drawResetClassNames() {
